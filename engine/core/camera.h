...
public:
	...
	void ProcessKeyboard(Camera_Movement direction, float deltaTime)
	{
		...
		if (direction == FORWARD)
			...
			Position -= Up * velocity;
		//printf("Camera Position Now : x {%f}  y {%f}  z {%f} \n", Position.x, Position.y, Position.z);
	}
...
inline void ProcessInput(Camera& camera, float _deltatime)
{
	if (InputManager::GetInstance().GetIsKeyDown(KEY::KEY_W))
	{
		camera.ProcessKeyboard(Camera_Movement::FORWARD, _deltatime);
	}
	if (InputManager::GetInstance().GetIsKeyDown(KEY::KEY_S))
	{
		camera.ProcessKeyboard(Camera_Movement::BACKWARD, _deltatime);
	}
	if (InputManager::GetInstance().GetIsKeyDown(KEY::KEY_A))
	{
		camera.ProcessKeyboard(Camera_Movement::LEFT, _deltatime);
	}
	if (InputManager::GetInstance().GetIsKeyDown(KEY::KEY_D))
	{
	...
	if (InputManager::GetInstance().GetIsKeyDown(KEY::KEY_E))
	{
		camera.ProcessMouseMovement(1, 0);
	}
